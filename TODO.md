...
Авторизация **ленивая и read-only**: запрашивается только на экспорте и только если реально найдены off-board карточки; `authorize({scope:'read', expiration:'never'})` вызывается прямым кликом (иначе браузер блокирует попап). Токен хранит клиентская библиотека Trello (member-private), у нас он не оседает. Статус кодируется префиксом к имени — отдельной колонки нет (по решению).
