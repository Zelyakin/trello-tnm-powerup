...
### 13. ✅ Anon-ключ давал полный CRUD ко всем данным всех досок — закрыто 2026-08-30
...
📋 **План реализации: [PLAN_SECURITY.md](PLAN_SECURITY.md)** (2026-08-29). Выбран вариант 2 в
supabase-нативной форме: обмен Trello-JWT на короткоживущий Supabase-токен в **Edge Function**
(не Cloudflare — так снимается асимметрия prod `pages.dev` / dev `github.io`), клеймы режутся
политиками RLS. Уточнение по факту: RLS не отсутствует — он включён с политиками `USING (true)`
на `boards`/`cards`/`time_entries` и выключен на `board_settings`.

**Закрыто 2026-08-30.** Реализован вариант 2 в supabase-нативной форме: Edge Function `trello-auth`
проверяет подпись Trello-JWT (RS256) и выпускает часовой токен с клеймом `trello_board_id`, доступ
режут политики RLS для роли `authenticated`. У `anon` не осталось ни политик, ни грантов — тот самый
`curl`, которым в июле читалась чужая запись `time_entries`, теперь отдаёт **401**.

Проверено на живой базе ([tests/rls_live_test.ts](tests/rls_live_test.ts)): токен доски видит только
её данные, токен несуществующей доски — ноль строк, вставка на чужую доску — 403, anon — 401 на все
четыре таблицы. Security Advisor: 0 ошибок, 0 предупреждений (кроме несвязанного Postgres-патча).

Ход работы и обоснование решений — [PLAN_SECURITY.md](PLAN_SECURITY.md), архитектура —
`CLAUDE.md` → «Authorization (v3.6)». Пункт 3 (отдельные ключи prod/dev) снят как потерявший смысл:
дев-плагин получает токен ровно на ту доску, где открыт.
